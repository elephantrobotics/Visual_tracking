...
p = ElephantRobot("192.168.1.28", 5001)
p.start_client()
...
class camera_detect:
    ...
    def __init__(self, camera_id, marker_size, mtx, dist):
        ...
        self.origin_mycbot_level = [0, -90, 120, -120, -90, 0]
    ...
    def Matrix_identify(self, ml):
        ml.write_angles(self.origin_mycbot_level, 1000)  # 移动到观测点
        self.wait()
        ...
        ml.write_coord(0, coords[0] + 50, 1000)
        self.wait()
        Mc2,tbe2,pos2 = self.reg_get(ml)
        ml.write_coord(2, coords[2] + 20, 1000)
        self.wait()
        Mc3,tbe3,pos3 = self.reg_get(ml)
        ml.write_coord(1, coords[1] + 20, 1000)
        self.wait()
        Mc4,tbe4,pos4 = self.reg_get(ml)
        ml.write_coord(0, coords[0] + 20, 1000)
        self.wait()
    ...
    def Eyes_in_hand_calibration(self, ml):
        ...
        ml.set_free_move_mode(1)
        input("focus servo and get current coords")
        ml.set_free_move_mode(0)
        ml.start_robot()
...
if __name__ == "__main__":
    ...
    #1.请根据readme中的指引修改id打开相机
    m.camera_open_loop()

    #2.根据readme2.2中的指引修改串口或IP地址，根据视频中的指引进行手眼标定
    # m.Eyes_in_hand_calibration(p)

    #3.完成手眼标定后，可验证坐标是否正确
    # m.stag_robot_identify_loop(p)
